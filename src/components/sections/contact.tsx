                ...
                <form className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      placeholder={language === 'en' ? "How can we help you?" : "আপনার বার্তা লিখুন..."}
                    />
                  </div>

                  <Button className="w-full h-12 rounded-xl text-base font-semibold shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all">
...
function ContactCard({ icon: Icon, title, value, href }: ContactCardProps) {
  const Content = (
    <div className="flex items-center gap-4 p-4 rounded-2xl bg-white dark:bg-white/5 border border-border/50 hover:border-primary/50 transition-colors group cursor-pointer">
      ...
      {href && <ArrowRight className="ml-auto size-5 text-muted-foreground group-hover:text-primary transition-colors" />}
    </div>
  );
